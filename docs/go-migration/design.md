...
- **D12** Privilege model: if the running process can write `/etc/hosts` directly (root, or appropriate ACL), do it silently. Otherwise, re-exec the binary under `sudo` for **only** the apply step, via a hidden `__apply-privileged` subcommand that reads the pre-rendered payload from a pipe (fd), re-validates it, and performs the atomic write. The `~/.hosts` write never requires sudo. macOS uses `sudo` (no `pkexec`).
...
5. If no → re-exec `sudo $0 __apply-privileged --payload-fd=<n>`; the hidden `__apply-privileged` subcommand reads the rendered bytes from the inherited pipe fd, re-validates them as a well-formed hostie-managed hosts block, and performs the same atomic write. Payload travels over a pipe (not argv, not tempfile) so it is invisible in `ps` and never lands on disk world-readable.
